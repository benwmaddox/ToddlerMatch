...
The Stasis migration keeps this loop and uses recorded prompts with bounded procedural feedback audio. Prompt audio is requested after the first successful audio initialization, including when a browser requires a pointer gesture to unlock audio, and tapping the prompt replays its voiceover. Dynamic labels and all numbers remain runtime text rendered from the checked-in Basic font. Like the web version, number rules use a generic `123` goal mark so the prompt does not reveal the answer; target and option cards display their real values. The runtime `MENU` control exits an active level and opens settings from the level screen.
